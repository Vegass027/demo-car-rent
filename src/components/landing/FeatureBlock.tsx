...
export function FeatureBlock({
  ...
  return (
    ...
    >
      {/* Заголовок + описание — на всю ширину сверху */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.3 }}
        transition={{ duration: 0.6, ease: EASE }}
        className="mb-8 md:mb-12 flex items-start gap-4"
      >
        ...
        <div className="flex flex-col gap-3">
          <h2 className="font-head text-3xl font-semibold leading-[1.1] tracking-[-0.02em] text-[#111] md:text-4xl lg:text-[48px]">
            {title}
          </h2>
          <p className="max-w-[68ch] text-base leading-[1.6] text-[#2F3437] md:text-lg">
            {description}
          </p>
        </div>
      </motion.div>
      ...
      >
        {/* Картинка — 12/12 если нет блока пунктов, иначе 8/12 */}
        <motion.div
          initial={{ opacity: 0, y: 32 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, amount: 0.15 }}
          transition={{ duration: 0.7, ease: EASE }}
          className={footer ? 'md:col-span-8' : 'md:col-span-12'}
        >
